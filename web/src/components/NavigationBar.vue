...
<template>
  <nav :class="['navigation', `navigation-${menu.navigation.type}`]"
    ...
    <button v-for="(item, index) in items" :key="item.pageId" data-menu-control
      ...
      :disabled="item.disabled" @click="choose(item, index)">
  ...
  <div v-if="menu.navigation.type === 'stepper'" class="stepper-actions">
    <button v-if="currentIndex > 0" data-menu-control class="stepper-back" @click="step('back')">{{ menu.navigation.backLabel || 'Back' }}</button>
    <button data-menu-control class="stepper-next" @click="step('next')">{{ currentIndex === items.length - 1 ? (menu.navigation.finishLabel || 'Finish') : (menu.navigation.nextLabel || 'Next') }}</button>
  </div>
</template>
